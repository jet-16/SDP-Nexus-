  ...
  <body>
    ...
    <section class="store-section">
      <div class="menu-div">
        ...
        <form class="searchbar-form" method="post" action="MOBA.php">
      ...
      <div class="title-div">
        <img src="images/dota_heroes2.jpg">
        <h1>MOBA Games</h1>
      </div>


        <div class="all-games-div">
          <div class="allgames-div"data-aos="fade-up"
          data-aos-offset="-200"
          ...
          data-aos-anchor-placement="top-center">
            ...
            <?php
              ...
              $exist = mysqli_query($con, "SELECT * FROM games WHERE game_category = 'MOBA' ");
              if (mysqli_num_rows($exist) > 0) {
			        if ($search == NULL)
			        {
                $result = mysqli_query($con,"Select * from games join users on users.user_id = games.game_seller where games.game_category ='MOBA'and games.game_status = 1");
                ...
                while($row = mysqli_fetch_array($result))
                {
      	   	...
      	   	{
               $result = mysqli_query($con,"Select * from games join users on users.user_id = games.game_seller where games.game_name like '%" .$search. "%' and games.game_category ='MOBA' and games.game_status = 1");
               while($row = mysqli_fetch_array($result))
               {
                 component($row['game_name'], $row['game_price'], $row['game_ID'], $row['game_photopath'], $row['game_videopath'], $row['user_username'],$row['game_releasedate'], $page);
      ...
      elseif (mysqli_num_rows($exist) === 0) {
        echo "<div class='nogames'>
          <h1>There are currently no games available for this category.</h1>
        </div>";
      }?>
