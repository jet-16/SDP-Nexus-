  ...
  <body>
    ...
    <section class="store-section">
      <div class="menu-div">
        ...
        <form class="searchbar-form" method="post" action="Sports.php">
        ...
        <div class="all-games-div">
          ...
          data-aos-anchor-placement="top-center">
            ...
            <?php
              ...
              $exist = mysqli_query($con, "SELECT * FROM games WHERE game_category = 'Sports' ");
              if (mysqli_num_rows($exist) > 0) {
			        if ($search == NULL)
			        {
                $result = mysqli_query($con,"Select * from games join users on users.user_id = games.game_seller where games.game_category ='Sports'and games.game_status = 1");
                ...
                while($row = mysqli_fetch_array($result))
                {
      	   	...
      	   	{
               $result = mysqli_query($con,"Select * from games join users on users.user_id = games.game_seller where games.game_name like '%" .$search. "%' and games.game_category ='Sports' and games.game_status = 1");
               while($row = mysqli_fetch_array($result))
               {
                 component($row['game_name'], $row['game_price'], $row['game_ID'], $row['game_photopath'], $row['game_videopath'], $row['user_username'],$row['game_releasedate'], $page);
